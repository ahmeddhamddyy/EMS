...
const MilitaryData = () => {
  ...
  // كود التعبئة التلقائية عند التعديل
  useEffect(() => {
      ...
      <div className="max-w-6xl mx-auto bg-white shadow-2xl rounded-3xl overflow-hidden border-[6px] border-[#1a2e2a]">
        ...
        <div className={`${isEditMode ? 'bg-blue-900' : 'bg-[#1a2e2a]'} p-6 text-white flex justify-between items-center border-b-[6px] border-yellow-600 transition-colors`}>
          <div className="flex items-center gap-4">
            <img src={RG_LOGO} alt="Egypt" className="w-12 shadow-md border border-white/20" />
            <h1 className="text-3xl font-black italic text-yellow-500 decoration-white underline-offset-8">
              {isEditMode ? `تعديل سجل: ${formData.name}` : "1. سجل البيانات العسكرية - الكتيبة ٥"}
            </h1>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8">
          <div className="flex flex-col items-center mb-10">
            ...
            <p className="text-[#1a2e2a] mt-2 font-black text-[10px] uppercase">{isEditMode ? "تغيير صورة المحارب" : "انقر لرفع صورة الفرد"}</p>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full ${isEditMode ? 'bg-blue-800' : 'bg-[#1a2e2a]'} text-yellow-500 py-6 rounded-2xl font-black text-3xl hover:bg-black shadow-2xl transition-all border-b-[10px] ${isEditMode ? 'border-blue-950' : 'border-yellow-700'} active:translate-y-2 active:border-b-0 flex items-center justify-center gap-4 mt-8 group`}
          >
            {loading ? <Loader2 className="animate-spin" size={35} /> : (
              <>
                <span>{isEditMode ? "حفظ التعديلات العسكرية" : "حفظ والانتـقال للبيانات الشخصية"}</span>
